#!/usr/bin/env python3
"""Benchmark RL strategy selector vs rule-based selector.

Usage:
    python3 scripts/benchmark_rl_selector.py --model models/rl_selector_v1.pkl --data models/rl_training_data.json
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark RL selector vs rule-based selector"
    )
    parser.add_argument(
        "--model",
        type=str,
        required=True,
        help="Path to trained RL model (.pkl)",
    )
    parser.add_argument(
        "--data",
        type=str,
    ...
    if not model_path.exists():
        ...
        logger.info("Train a model first with train_rl_selector.py")
        sys.exit(1)

    logger.info(f"Loading trained model from: {args.model}")

    try:
        config = RLConfig(model_save_path=str(model_path))
        rl_selector = RLStrategySelector(config)
    ...
    rl_metrics = benchmark_selector(rl_selector_fn, test_data, name="RL Selector")
    rule_metrics = benchmark_selector(rule_based_selector_fn, test_data, name="Rule-Based Selector")

    # Print comparison
    print("\n┌─────────────────────────────────┬───────────────────┬───────────────────┐")
    print("│ Metric                          │ RL Selector       │ Rule-Based        │")
    print("├─────────────────────────────────┼───────────────────┼───────────────────┤")
    print(f"│ Avg Final KPI                   │ {rl_metrics['avg_final_kpi']:17.4f} │ {rule_metrics['avg_final_kpi']:17.4f} │")
    print(f"│ Avg Rounds                      │ {rl_metrics['avg_rounds']:17.2f} │ {rule_metrics['avg_rounds']:17.2f} │")
    print(f"│ Avg Strategy Switches           │ {rl_metrics['avg_strategy_switches']:17.2f} │ {rule_metrics['avg_strategy_switches']:17.2f} │")
    print(f"│ Convergence Rate                │ {rl_metrics['convergence_rate']:16.1%} │ {rule_metrics['convergence_rate']:16.1%} │")
    print(f"│ Target Reached Rate             │ {rl_metrics['target_reached_rate']:16.1%} │ {rule_metrics['target_reached_rate']:16.1%} │")
    print("└─────────────────────────────────┴───────────────────┴───────────────────┘")
    ...
    if kpi_improvement > 0 or rounds_reduction > 0:
        print("\n✅ RL selector shows improvement!")
    else:
